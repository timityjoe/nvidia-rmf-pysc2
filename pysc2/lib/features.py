...
class Features(object):
  # Mod by Tim: Prevent error msgs from flooding the terminal
  error_msg_counter = 0
  ...
  def available_actions(self, obs):
    ...
    for a in obs.abilities:
      if a.ability_id not in actions.ABILITY_IDS:
        # ---------------------------------------
        # Mod by Tim: Prevent this error from flooding the terminal
        # logging.warning("Unknown ability %s seen as available.", a.ability_id)
        # continue
        self.error_msg_counter = self.error_msg_counter + 1
        if self.error_msg_counter == 200:
          logging.warning("Unknown ability %s seen as available.", a.ability_id)
          self.error_msg_counter = 0
        continue
        # ---------------------------------------
