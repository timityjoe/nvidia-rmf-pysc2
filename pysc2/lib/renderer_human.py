...
class RendererHuman(object):
  ...
  def __init__(self, fps=22.4, step_mul=1, render_sync=False,
    ...
    logger.info(f"self._render_thread:{self._render_thread}")
  ...
  def init_window(self):
    ...
    # --------------------------------------------------
    # Mod by Tim: Some sanity checks, solve the error
    # line 1731 - pygame.display.flip()
    # pygame.error: Unable to make GL context current
    # See 
    # https://www.pygame.org/docs/ref/display.html?highlight=gl%20context
    # --------------------------------------------------
    ...
    logger.info(f"1) main_screen_px:{main_screen_px}, _rgb_screen_px:{self._rgb_screen_px}, _feature_screen_px:{self._feature_screen_px}")

    window_size_ratio = main_screen_px
    num_feature_layers = 0
    if self._render_feature_grid:
      logger.info("2) _render_feature_grid")
    ...
    # ----------------------------------------------------------
    # Mod by Tim:
    # See https://github.com/pygame/pygame/blob/main/docs/reST/ref/display.rst
    # for flags description
    # See https://www.pygame.org/docs/tut/DisplayModes.html for display mode settings
    # self._window = pygame.display.set_mode(window_size_px, 0, 32)
    # Set the display mode
    winstyle = 0  # |FULLSCREEN
    bestdepth = pygame.display.mode_ok(window_size_px, winstyle, 32)
    # flags = 0
    # flags = (pygame.OPENGL | pygame.DOUBLEBUF | pygame.RESIZABLE)
    flags = pygame.RESIZABLE
    # flags = pygame.OPENGL | pygame.DOUBLEBUF
    # flags = (pygame.OPENGL | pygame.DOUBLEBUF | pygame.RESIZABLE | pygame.HWSURFACE )
    print("pygame.OPENGL:", pygame.OPENGL, " DOUBLEBUF:", pygame.DOUBLEBUF, " pygame.RESIZABLE:", pygame.RESIZABLE)
    # print("pygame.display.get_driver:", pygame.display.get_driver(), " window_size_px:", window_size_px, " flags:", flags, " bestdepth:", bestdepth)
    print("pygame.display.get_driver:", pygame.display.get_driver(), " window_size_px:", window_size_px, " bestdepth:", bestdepth)
    print("pygame.display.Info:", pygame.display.Info())
    
    modes = pygame.display.list_modes(bestdepth)
    print("pygame.display.list_modes len:", len(modes))
    for i in modes:
      print("Mode:", i[0], " ", i[1])

    if not modes:
      print(bestdepth, '-bit not supported')
    else:
      print('Found Resolution:', modes[0])

    # self._window = pygame.display.set_mode(window_size_px, 0, 32)
    # self._window = pygame.display.set_mode((1920,1080), 0, 32)
    # self._window = pygame.display.set_mode(window_size_px, winstyle, bestdepth)
    # self._window = pygame.display.set_mode(window_size_px, flags, bestdepth)
    self._window = pygame.display.set_mode(window_size_px, flags, 32)
    # self._window = pygame.display.set_mode(modes[0], flags, 32)
    # ----------------------------------------------------------
    ...
    def add_surface(surf_type, surf_loc, world_to_surf, world_to_obs, draw_fn):
      # logger.info("add_surface() surf_type:{surf_type}")
    ...
    logger.info(f"  self._feature_screen_px:{self._feature_screen_px}")
    if self._feature_screen_px:
      logger.info("3) _feature_screen_px")
    ...
    # logger.info(f"  self._rgb_screen_px:{self._rgb_screen_px}")
    if self._rgb_screen_px:
      logger.info("4) _rgb_screen_px")
    ...
    if self._render_rgb:
      logger.info(f"5a) self._render_rgb:{self._render_rgb}, 1)rgb_screen_to_main_screen...")
      ...
      logger.info(f"5b) self._render_rgb:{self._render_rgb}, 2)feature_screen_to_main_screen...")
    ...
    if self._render_feature_grid and num_feature_layers > 0:
      logger.info(f"6) _render_feature_grid:{self._render_feature_grid}, num_feature_layers:{num_feature_layers}")
      ...
      def add_layer(surf_type, world_to_surf, world_to_obs, name, fn):
        # logger.info(f"add_layer() surf_type:{surf_type}")
        """Add a layer surface."""
      ...
      def add_raw_layer(from_obs, name, color):
        # logger.info(f"add_raw_layer() name:{name}")
      ...
      if self._game_info.options.raw:
        logger.info(f"7)  _game_info.options.raw:{self._game_info.options.raw}")
      ...
      if self._feature_minimap_px:
        logger.info(f"8)  _feature_minimap_px:{self._feature_minimap_px}")
      ...
      if self._feature_screen_px:
        logger.info(f"9)  _feature_screen_px:{self._feature_screen_px}")
    ...
    for act in self._past_actions:
      if act.pos and now < act.deadline:
        ...
        if isinstance(act.pos, point.Point):
          ...
          # logger.info(f" isinstance size:{size}")
        else:
  ...
  def render(self, obs):
    # Mod by Tim:
    # print("renderer_human.py render()")
    """Push an observation onto the queue to be rendered."""
    if not self._initialized:
      logger.info("X) not self._initialized, returning...")
      return
    now = time.time()
  ...
  def render_thread(self):
    # Mod by Tim:
    # print("renderer_human.py render_thread()")
    ...
    while obs:  # Send something falsy through the queue to shut down.
      ...
      if obs:
        ...
        if self._obs_queue.empty():
          ...
          # Mod by Tim: "Unable to make GL context current" happens here
          self.render_obs(obs)
        if self._video_writer:
  ...
  def render_obs(self, obs):
    # Mod by Tim:
    # print("renderer_human.py render_obs()")
    ...
    # logger.info(f"  self._surfaces size:{len(self._surfaces)}")
    for surf in self._surfaces:
      # Render that surface.
  ...
  def run(self, run_config, controller, max_game_steps=0, max_episodes=0,
    ...
    try:
      while True:
        ...
        controller.step()
        logger.info(f"W1) num_episodes:{num_episodes}")
        ...
        while True:
          ...
          if cmd == ActionCmd.STEP:
            ...
            if not is_replay and save_replay:
              self.save_replay(run_config, controller)
              logger.info("X2) return...")
            return
          elif cmd == ActionCmd.RESTART:
            logger.info("X3) restart...")
            break
          else:
            logger.info("X4) unexpected cmd...")
