...
class StarcraftProcess(object):
  ...
  def __init__(self, run_config, exec_path, version, full_screen=False,
    ...
    if FLAGS.sc2_gdb:
      ...
      print("StarcraftProcess init():", args)
    ...
    try:
      with sw("startup"):
        if not FLAGS.sc2_port:
          ...
          print("sc_process init() Sleeping 10 sec, waiting for SC2 run up..")
          time.sleep(10)
        if connect:
          print("sc_process init() Connecting to remote controller, ")
  ...
  def _launch(self, run_config, args, **kwargs):
    print("sc_process launch():")
    """Launch the process and return the process object."""
    del kwargs
    try:
      with sw("popen"):
        print(" args:", args)
        print(" cwd:", run_config.cwd)
        print(" env:", subprocess.DEVNULL)
        # Mod by Tim: Copy from python-sc2 code
